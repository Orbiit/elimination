...
urlToPage global url =
    case url.query of
        Just query ->
            let
                path =
                    case List.head (String.split "&" query) of
                        Just str ->
                            str
            ...
            if String.startsWith "@" path then
                ...
                let
                    game =
                        String.dropLeft (String.length "settings!") path
                in
                Command <|
                    Cmd.batch
...
title model =
    case model.page of
        ...
        Pages.GameSettings ->
            if model.gameSettings.game == Nothing then
                "Create a game"
...
view model =
    { title =
        let
            unread =
                model.header.notifs.unread

            append =
                (if unsavedChanges model then
                    "*"
...
update msg model =
    case msg of
        ...
        BaseMsg subMsg ->
            let
                ( subModel, subCmd, pageCmd ) =
                    Base.update subMsg model model.header
            in
        ...
        UserSettingsMsg subMsg ->
            let
                ( subModel, subCmd, pageCmd ) =
                    Pages.UserSettings.update subMsg model model.userSettings
            in
        ...
        UserMsg subMsg ->
            let
                ( subModel, subCmd, pageCmd ) =
                    Pages.User.update subMsg model model.user
            in
        ...
        FrontPageMsg subMsg ->
            let
                ( subModel, subCmd, pageCmd ) =
                    Pages.FrontPage.update subMsg model model.frontPage
            in
        ...
        GameSettingsMsg subMsg ->
            let
                ( subModel, subCmd, pageCmd ) =
                    Pages.GameSettings.update subMsg model model.gameSettings
            in
        ...
        GameMsg subMsg ->
            let
                ( subModel, subCmd, pageCmd ) =
                    Pages.Game.update subMsg model model.game
            in
        ...
        OnKeyDown key ->
            case key of
                "Escape" ->
                    let
                        ( header, _, _ ) =
                            Base.update Base.Close model model.header

                        ( frontPage, _, _ ) =
                            Pages.FrontPage.update Pages.FrontPage.HideModal model model.frontPage

                        ( gameSettings, _, _ ) =
                            Pages.GameSettings.update Pages.GameSettings.HideModal model model.gameSettings

                        ( game, _, _ ) =
                            Pages.Game.update Pages.Game.HideModal model model.game
                    in
                    ( { model
                        | askDiscardChanges = Nothing
                        , header = header
