        ...
        InfoLoaded gameID result ->
            case result of
                Ok game ->
                    let
                        description =
                            Markup.markup Nothing game.description
                    in
                ...
                String.concat
                    [ case ( player.killTime, player.killer, player.killerName ) of
                        ( Just time, Just killer, Just killerName ) ->
                            "Eliminated on "
                                ++ HumanTime.display global.zone time
                                ++ " by "
                                ++ killerName
    ...
    [ article [ A.class "main content profile" ]
        [ div [ A.class "profile-info" ]
            ...
            , p [ A.class "profile-desc profile-stats" ]
                [ text
                    (Api.gameStateNameWithTime global.zone model.info.state model.info.time
                        ++ " "
                        ++ char Middot
                        ++ " "
                        ++ String.fromInt (List.length (List.filter (\player -> player.alive) model.info.players))
                        ++ " of "
                        ++ String.fromInt (List.length model.info.players)
                        ++ (if List.length model.info.players == 1 then
                                " participant"
