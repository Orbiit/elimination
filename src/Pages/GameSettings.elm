...
import Html.Events exposing (onCheck, onClick, onSubmit, stopPropagationOn)
    ...
    div [ A.class "member-item" ]
        ([ div [ A.class "member" ]
            ...
            , span [ A.class "member-info" ]
                [ text
                    ("Joined "
                        ++ HumanTime.display zone player.joined
                        ++ " "
                        ++ char Middot
                        ++ " "
...
hasUnsavedChanges model =
    model.name.value
        ...
        /= model.password.original
        || Tuple.first model.joinable
        /= Tuple.second model.joinable
        ...
        [ h1 []
            (Utils.filter
                ...
                , case model.game of
                    Just gameID ->
                        Just
                            (a [ A.class "button", A.href ("?!" ++ gameID) ]
                                [ text "View game page" ]
                            )
