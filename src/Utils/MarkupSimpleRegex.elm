...
import Html exposing (Html, a, em, strong, text)
...
urlRegex =
    Utils.makeRegex "\\bhttps?://[-A-Za-z0-9+@#/%?=~_|!:,.;&]*[-A-Za-z0-9#/=_]|@[a-z0-9_-]{3,20}(?=[^\\w-])|![0-9a-f]{5}|\\*{1,2}|\\\\."
                            ...
                            [ text <|
                                case state.gameNames of
                                    Just names ->
                                        let
                                            maybeName =
                                                Dict.get (String.dropLeft 1 match.match) names
                                        in
                                        case maybeName of
                                            Just name ->
                                                name
